...
public class NewOrderItemAdapter extends RecyclerView.Adapter<NewOrderItemAdapter.Hold> {

    List<OrderItemsModel> oiList;
    Context context;
    double totPrice = 0.0, price = 0.0;
    int qty = 0;

    public NewOrderItemAdapter(Context context, List<OrderItemsModel> items) {
    ...
    @NonNull
    @Override
    public NewOrderItemAdapter.Hold onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View mView = LayoutInflater.from(parent.getContext()).inflate(R.layout.ongoing_order_item_row, parent, false);
        return new Hold(mView);
    }

    @Override
    public void onBindViewHolder(@NonNull NewOrderItemAdapter.Hold holder, int position) {

        OrderItemsModel oim = oiList.get(position);
